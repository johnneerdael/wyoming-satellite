...
class SatelliteBase:
    ...
    def __init__(self, settings: SatelliteSettings) -> None:
        ...
        # Audio handling
        self._current_tts_chunks: List[AudioChunk] = []
        self._temp_wav_dir = Path("/tmp/wyoming_satellite")
        self._temp_wav_dir.mkdir(exist_ok=True)
    ...
    async def _snd_task_proc(self) -> None:
        """Audio handling loop using catt."""
        while self.is_running:
            try:
                ...
                if AudioChunk.is_type(event.type):
                    ...
                    self._current_tts_chunks.append(chunk)

                elif AudioStop.is_type(event.type) and self._current_tts_chunks:
                    ...
                    try:
                        ...
                        if process.returncode != 0:
                            ...
                            _LOGGER.error("Failed to cast audio with catt: %s (code=%d)", 
                                        error_msg, process.returncode)
                            # Don't trigger played event on error
                            return
                        elif snd_event.is_tts:
                            await self.trigger_played()

                    except Exception as e:
                        _LOGGER.exception("Error playing audio with catt: %s", str(e))
                    finally:
                        # Clean up
                        try:
                            temp_wav.unlink()
                            ...
                            pass
                        self._current_tts_chunks.clear()

            except asyncio.CancelledError:
                break
            except Exception:
                _LOGGER.exception("Unexpected error in audio handling task")
    ...
    async def _play_wav(
        ...
        """Send WAV to be played via catt cast."""
